...
post_samps <- as.matrix(samples)

# Assuming 'gamma' represents the random slopes for all patients, you might need to adjust the name based on your output
gamma_post_samps <- post_samps[, grepl("gamma", colnames(post_samps))]
gamma_post_samps

gamma_summary <- apply(gamma_post_samps, 2, summary)
print(gamma_summary)

gamma_means <- colMeans(gamma_post_samps)
patient_ids <- 1:length(gamma_means) # Adjust based on your data structure
plot_data <- data.frame(Patient = patient_ids, GammaMean = gamma_means)

ggplot(plot_data, aes(x = Patient, y = GammaMean)) +
  geom_point() +
  geom_errorbar(aes(ymin = GammaMean - 2*sqrt(gamma_summary["Var"]), ymax = GammaMean + 2*sqrt(gamma_summary["Var"])), width = 0.1) +
  theme_minimal() +
  labs(title = "Rate of Change of SGAJ for Each Patient", x = "Patient ID", y = "Rate of Change (Gamma)")

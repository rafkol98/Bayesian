...
sgaj_miss_patient_12 <- sgaj_miss[sgaj_miss$patient == 12,] %>% mutate(index = 1:6)

samples_miss_mu_pp <- summary(
  subset_draws(samples_miss_draws, c(paste0("mu[",ids_patient_12,"]"),
                            paste0("yrep[",ids_patient_12,"]"))),
  ~quantile(.x, probs = c(0.025,0.5,0.975))
) %>% mutate(index = rep(1:nrow(sgaj_miss_patient_12),2))


combined_df_patient_12 <- samples_miss_mu_pp %>% left_join(sgaj_miss_patient_12, by="index")  %>%
  mutate(
...
ggplot(combined_df_patient_12, aes(x=time, y=measured)) + geom_point() + theme_classic()
```

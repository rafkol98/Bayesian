...
$$
\\ \mu_{it} = \alpha_i + \beta_{age}\left(age_i - 56 \right)/10 + \beta_{time}\left(time_t - 12 \right)/24 + \beta_{female}(female_i)
...
\\ \beta_{time} \sim \mathcal{N}(0, 10^2)
\\ \beta_{female} \sim \mathcal{N}(-0.1, 10^2)
$$
- For the $\beta_{age}, \beta_{time}$, and $\beta_{female}$ as the brief explains, the effect is not currently understood well enough and as such I am assuming vague priors, allowing the data to play a primary role in informing these relationships. 
- For the prior for $\beta_{female}$ I am subtly incorporating the expectation that SGAJ levels might be slightly lower in females than in males (as explained by the principal investigator).
...
model <- "model{
  ...
  for (i in 1:n) {
    mu[i] <- alpha[patient[i]] + beta_age*(age[i] - 56)/10 + beta_time*(time[i] - 12)/24 + beta_female*female[i]
...
```{r}
for(patient in unique(sgaj$patient)) {
  print(summary(sgaj[sgaj$patient == patient,]$measureds))
}
```
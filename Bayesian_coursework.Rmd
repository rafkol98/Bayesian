...
The rate of change per day of SGAJ for patients is assumed to follow a normal distribution with mean $\mu_{it}$ and variance $\sigma^2$. The expected SGAJ measurement for patient `i` at time `t` is modeled as $\mu_{it}$.
$$
\\ \mu_{it} = \alpha_i + \beta\left(age_i - 56 \right)/10 + \gamma\left(time_t - 12 \right)/24 + \delta(female_i)
\\ y_{it} \sim \mathcal{N}(\mu_{it}, \psi^2)
$$
The variability between patients $i$ is represented by a random effect $a_i$ (patient-specific random intercept), which is assumed to be drawn from a Normal distribution with mean $\mu_a$ and variance $\sigma_a^2$. 
...
# This samples variable now holds 'n' samples from a Half-Normal(0, 90^2) distribution
summary(samples)
```
## rjags
...
model <- "model{
  ...
  for (i in 1:n) {
    for(t in 1:T) {
       mu[i,t] = alpha[patient[i]] + beta*(age[i]-56)/10 + gamma*(time[j]-12)/24 + delta*female[i]
  ...
  for (p in 1:n_patients) {
    alpha[p] ~ dnorm(mu_patient, prec_patient)
  }
  ...
  mu_a ~ dnorm(250, 1/(20.41^2))

  # Prior for the standard deviation of the random intercepts
  sigma_patient ~ dnorm(0, 1/(90^2))T(0, )
  prec_patient <- 1/(sigma_patient^2)

  # Prior for the measurement error variance
  sigma ~ dunif(0, 150)
  tau <- 1/(sigma^2)
}"

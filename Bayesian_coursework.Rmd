...
$$
\\ \mu_{it} = \alpha_i + \beta_{age}\left(age_i - 50 \right)/10 + \beta_{female}(female_i) + \gamma_{i}\left(time_t - 12 \right)/24
...
\\ \sigma_a \sim \mathcal{N}(90, 5.1^2)
$$
...
# TODO: fix precise wording
- For the random effects standard deviation $\sigma_a$, I am assuming that it follows a Normal distribution with mean 90 (value influenced by the information from the brief), with a standard deviation of (100-80)/(1.96*2)=5.1 to reflect that the value could be as low as 80 and as high as 100 (95% CI).

```{r}
set.seed(2292)  # Set seed for reproducibility
sigma_a_samples <- abs(rnorm(10000, mean=0, sd=90))
summary(sigma_a_samples)

# when normal with mean 90 but could be as low as 85 and as high as 95.
sd_sigma_a_norm <-(100-80)/(1.96*2)
sd_sigma_a_norm
sigma_a_norm_samples <- abs(rnorm(10000, mean=90, sd=sd_sigma_a_norm))
summary(sigma_a_norm_samples)
```
# TODO: why?
...
init_values_func <- function(seed) {
  ...
  return(
    list(
      ...
      mu_gamma = rnorm(1, 0, 10),
      sd_gamma = runif(1, 0, 10),

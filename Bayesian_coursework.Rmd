...
samples_miss_mu_pp <- summary(
  subset_draws(samples_miss_draws, c(paste0("mu[",data_miss$n,"]"),
                            paste0("yrep[",data_miss$n,"]"))),
...
```{r}
samples_miss_mu_pp[2,]

predicted_value <- samples_miss_mu_pp[[1,"50%"]]
sgaj_with_predicted <- sgaj_miss[sgaj_miss$patient == 12,]
sgaj_with_predicted[6,]$measured <- predicted_value

ggplot(sgaj_with_predicted, aes(x=time, y=measured)) + geom_point() + theme_classic()
```

...
for (patient in 1:12) {
  sgaj_patient <- sgaj[sgaj$patient==patient,]
  print(sgaj_patient)
  print(ggplot(sgaj_patient, aes(x=measured)) + 
    geom_density() +  
...
The rate of change per day of SGAJ for each patient is assumed to follow a normal distribution with mean $\mu_{it}$ and variance $\psi^2$. The expected SGAJ measurement for patient `i` at time `t` is modeled as $\mu_{it}$.
$$
